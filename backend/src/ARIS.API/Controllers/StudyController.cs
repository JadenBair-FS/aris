...
public class StudyController : ControllerBase
{
    ...
    /// <summary>
    /// Extracts clean signals from the supplied resume and job description texts, runs tier
    /// analysis via the knowledge graph, then calls Ministral to produce a tailored resume
    /// (summary + experience bullets) as plain text. Nothing is written to the database.
    /// </summary>
    [AllowAnonymous]
    [HttpPost("tailor")]
    public async Task<IActionResult> Tailor([FromBody] StudyCompareRequest request)
    {
        if (string.IsNullOrWhiteSpace(request.ResumeText))
            return BadRequest("ResumeText is required.");
        if (string.IsNullOrWhiteSpace(request.JobDescriptionText))
        ...
        _logger.LogInformation("StudyController.Tailor: starting signal extraction.");
    ...
    // RAG path: embed resume text, find top-20 skills from ref_skills, call Ministral.
    ...
    // GraphRAG path: extract + ground both signals, run tier analysis, call Mistral.
