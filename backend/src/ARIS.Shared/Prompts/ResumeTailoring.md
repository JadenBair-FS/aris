...
| T5 — Hard Gap | No graph path exists — not covered, not bridgeable | DO NOT write, name, paraphrase, imply, or hint at any T5 skill. Not even once. These skills are off limits in every form. |
...
3. HARD GAPS (T5) ARE ABSOLUTELY FORBIDDEN. The graph context block above lists them under "OFF LIMITS — never mention". Do not write any of those skill names. Do not describe what those skills do. Do not imply the candidate has experience with them. Any bullet that contains a T5 skill name — in any form, abbreviation, or synonym — is a critical failure.
4. Bullets are 1 to 2 lines, led by a strong action verb, quantified where the original was quantified.
5. Write in third person only. Do not use first-person pronouns (I, my, me, we, our). Use direct statements: "Developed...", "Led...", "Delivered..." — never "I developed...", "My work...".
6. Plain text only — no markdown, no asterisks, no bold, no italic, no bullet symbols, no headers, no special characters used for formatting.
7. Return the same number of bullets as the original entry. Do not add or remove bullets.
